...
- React Query provider wired in App layout for caching, retries, and deduplication

## Golden Rules

1. Do not use raw `fetch()` in components/hooks. Use the Ky helpers.
2. API responses may be raw or wrapped; the client auto‑unwraps. Consume as the actual payload.
3. Prefer React Query for server state (lists, detail, mutations). Local UI state remains with React state.
4. Defensive programming at the usage site: null checks and sane defaults to avoid runtime crashes.
...
const data = await getJson<{ events: Event[]; pagination: Pagination }>(`/api/events?${params}`);
...
export function useEvents(params: { page: number; limit: number }) {
  const search = new URLSearchParams({ page: String(params.page), limit: String(params.limit) }).toString();
  return useQuery({
    queryKey: eventsKeys.list(params),
    queryFn: () => getJson<{ events: any[]; pagination: any }>(`/api/events?${search}`),
    staleTime: 30_000,
...
- React Query provider is available in `src/app/[locale]/layout.tsx` via `ReactQueryProvider`.

## Query Key Utilities

- Use factories in `src/lib/api/query-keys.ts` for consistent, hierarchical keys.
- Examples:

````typescript
import { eventsKeys, usersKeys, bankAccountsKeys, rolesKeys } from "@/lib/api/query-keys";
...
- Normalize errors with `normalizeApiError()` and let components render friendly messages
...
- React Query DevTools are installed as a dev dependency and only render in development.
...
- Wrap any client subtree with this provider when building isolated sandboxes, otherwise the app layout includes it.
